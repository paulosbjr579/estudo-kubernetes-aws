...
echo "plugin /usr/lib64/openvpn/plugins/openvpn-plugin-auth-pam.so \"login login USERNAME password PASSWORD\"" >>/etc/openvpn/server.conf

echo 'push "dhcp-option DNS 8.8.8.8"' >>/etc/openvpn/server.conf
echo 'push "dhcp-option DNS 8.8.4.4"' >>/etc/openvpn/server.conf

echo 'push "route 10.20.0.0 255.255.0.0"' >>/etc/openvpn/server.conf
